...
class LineageExplorerService:
    ...
    def node_paths(
        ...
        return self._cached(
            namespace="node-paths",
            key_parts=[node_id, export_id or "<latest>", family or "<all>", str(limit)],
            ttl_seconds=self.settings.lineage_expansion_cache_ttl_seconds,
            loader=lambda: self.repo.get_node_paths(
                node_id=node_id,
                export_id=export_id,
                family=family,
                limit=limit,
    ...
    def node_audit_context(self, node_id: str, export_id: str | None = None) -> dict | None:
        return self._cached(
            namespace="node-audit-context",
            key_parts=[node_id, export_id or "<latest>"],
            ttl_seconds=self.settings.lineage_expansion_cache_ttl_seconds,
            loader=lambda: self.repo.get_node_audit_context(node_id=node_id, export_id=export_id),
        )
