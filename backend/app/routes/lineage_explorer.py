...
@router.get("/node/{node_id}/paths")
async def lineage_node_paths(
    response: Response,
    node_id: str,
    export_id: str | None = Query(None),
    family: str | None = Query(
        None,
        pattern="^(catalog_hierarchy|semantic_implements|technical_upstream|technical_downstream|usage_context)$",
    ),
    limit: int = Query(100, ge=1, le=1000),
):
    service = LineageExplorerService()
    result = service.node_paths(node_id=node_id, export_id=export_id, family=family, limit=limit)
    if result is None:
        raise HTTPException(status_code=404, detail="No migration_v2 paths found for this node")
    _apply_response_metadata(response, service)
    return result


@router.get("/node/{node_id}/audit-context")
async def lineage_node_audit_context(
    response: Response,
    node_id: str,
    export_id: str | None = Query(None),
):
    service = LineageExplorerService()
    result = service.node_audit_context(node_id=node_id, export_id=export_id)
    if result is None:
        raise HTTPException(status_code=404, detail="No migration_v2 audit context found for this node")
    _apply_response_metadata(response, service)
    return result
