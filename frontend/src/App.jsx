...
async function copyInspectorValue(value) {
  const plainText = inspectorValue(value);
  if (plainText === "-") return;
  await navigator.clipboard.writeText(plainText);
}

function DqcInspectorTable({ item }) {
  const copyableKeys = new Set(["matched_node_id", "matched_path_full"]);
  return (
    <div className="dq-inspector-table-wrap">
      <table className="dq-inspector-table">
        <tbody>
          {Object.entries(item || {}).map(([key, value]) => (
            <tr key={key}>
              <th>{key}</th>
              <td>
                <span>{inspectorValue(value)}</span>
                {copyableKeys.has(key) && value !== undefined && value !== null && value !== "" && (
                  <button
                    type="button"
                    className="dq-copy-button"
                    title={`Copy ${key}`}
                    aria-label={`Copy ${key}`}
                    onClick={() => copyInspectorValue(value)}
                  >
                    Copy
                  </button>
                )}
  ...
  return (
    <div className="dq-shell">
      ...
      <main className="dq-main">
        ...
        {selected && (
          <aside className="dq-inspector" ref={inspectorRef}>
            <div className="dq-card-head compact">
              <h3>Inspector</h3>
              <button onClick={() => setSelected(null)}>Close</button>
            </div>
            <DqcInspectorTable item={selected} />
          </aside>
        )}
      </main>
    </div>
