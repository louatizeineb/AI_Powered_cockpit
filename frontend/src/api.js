...
export async function fetchMigrationExports() {
  return request(() => api.get("/migration-v2/exports"));
}

export async function fetchMigrationOverview(exportId) {
  return request(() => api.get(`/migration-v2/exports/${encodeURIComponent(exportId)}/overview`));
}

export async function fetchMigrationQueue(exportId, { status = "", issueType = "", publishPolicy = "", limit = 200 } = {}) {
  return request(() => api.get(`/migration-v2/exports/${encodeURIComponent(exportId)}/validation-queue`, {
    params: {
      status: status || undefined,
      issue_type: issueType || undefined,
      publish_policy: publishPolicy || undefined,
      limit,
    },
  }));
}

export async function decideMigrationIssue(exportId, issueId, payload) {
  return request(() => api.post(
    `/migration-v2/exports/${encodeURIComponent(exportId)}/validation-queue/${encodeURIComponent(issueId)}/decision`,
    payload
  ));
}

export async function fetchMigrationActivity(exportId) {
  return request(() => api.get(`/migration-v2/exports/${encodeURIComponent(exportId)}/activity`));
}

export async function fetchMigrationAgentEvaluations(exportId) {
  return request(() => api.get(`/migration-v2/exports/${encodeURIComponent(exportId)}/agent-evaluations`));
}

export async function fetchSchemaIntelligence(exportId) {
  return request(() => api.get(`/migration-v2/exports/${encodeURIComponent(exportId)}/schema-intelligence`));
}

export async function fetchSchemaColumns(exportId, tableName) {
  return request(() => api.get(
    `/migration-v2/exports/${encodeURIComponent(exportId)}/schema-intelligence/${encodeURIComponent(tableName)}/columns`
  ));
}

export async function fetchGovernanceItems(exportId, { state = "quarantine", kind = "object", limit = 100 } = {}) {
  return request(() => api.get(`/migration-v2/exports/${encodeURIComponent(exportId)}/governance-items`, {
    params: { state, kind, limit },
  }));
}

export async function runMigrationAction(exportId, action, requestedBy, apiBaseUrl = API_BASE_URL) {
  return request(() => api.post(
    `/migration-v2/exports/${encodeURIComponent(exportId)}/actions/${encodeURIComponent(action)}`,
    { requested_by: requestedBy, api_base_url: apiBaseUrl }
  ));
}

export async function askGovernanceGraphRag(exportId, query, subject = null) {
  return request(() => api.post("/graphrag/governance/retrieve", {
    export_id: exportId,
    query,
    subject,
    limit: 20,
...
export async function askMigrationGovernanceAssistant(exportId, payload) {
  return request(() => api.post(
    `/migration-v2/exports/${encodeURIComponent(exportId)}/assistant/chat`,
    payload
  ));
}
